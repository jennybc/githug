...
test_that("repo_path detects invalid input", {
  expect_error(repo_path(letters[1:2]), "length\\(x\\) == 1L is not TRUE")
  expect_error(repo_path(1), "is.character\\(x\\) is not TRUE")
})

test_that("nonexistent directory", {
  tpath <- tmp_repo_path()
  expect_error(repo_path(tpath), "directory does not exist")
...
test_that("dir exists but is not a git repo", {
  tpath <- tmp_repo_path()
  dir.create(tpath)
  expect_error(repo_path(tpath), "no git repo exists here")
  expect_false(is_in_repo(tpath))
  expect_false(is_a_repo(tpath))
})
...
test_that("dir is descendant of a git repo", {
  ...
  expect_error(repo_path(nested_path, ceiling = 0), "no git repo exists")
  expect_false(is_in_repo(nested_path, ceiling = 0))
  expect_false(is_in_repo(nested_path, ceiling = 1))

  ## walk one level up
  expect_error(repo_path(nested_path, ceiling = 1), "no git repo exists")
  expect_false(is_in_repo(nested_path, ceiling = 1))
...
test_that("we default to the obvious repo", {
  ...
  expect_identical(tpath, repo_path())
